...
/*
 * Created using IntelliJ IDEA. Date: Apr 27, 2004 Time: 11:36:45 PM
 * 
 * @author Miguel Mu\u00f1oz
 *         <p/>
...
public enum SpringUtilities
{
   ...
   * <p><b>Note:</b> <i>This must be called after all components have been added
   * to the parent.</i>
	 * 
	...
	{
		SpringLayout layout;
		try { layout=(SpringLayout) parent.getLayout(); }
		catch (ClassCastException exc) { parent.setLayout(layout = new SpringLayout()); }
   ...
   * to the parent.</i>
    ...
    // If there aren't enough, I could conceivably add blank components until 
    // the number is right, but I'm not going to bother for now.
    assert parent.getComponentCount() == rows*cols 
        : "Wrong Component Count: " + parent.getComponentCount()  + " != "
          + rows + "*" + cols; 
