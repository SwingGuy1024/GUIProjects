...
// This class no longer works, now that sun.swing.SwingUtilities2 is no longer
// available. It wasn't a good way to do this anyway. A better way would have
// been to extend one of the subclasses of JTextComponent that are intended
// for substring rendering details, like color.
 ...
 *
 * todo:    Two bugs to fix:
 * todo     1) In HiliteView, search for &
 * todo     2) In HiliteView, search for <font> (without the space before)
 *
 * @author Miguel Mu\u00f1oz
 *         <p/>
 ...
 * <p/>
 * todo: Add listener code, so we won't need to specify the table model on
 * todo  construction. The renderer would listen to events generated by the
 * todo  JTextField.
 */
public class HilightRenderer extends DefaultTableCellRenderer {
	...
	private static EncodedSearcher mGt = new EncodedSearcher(">", "&gt;");

	/**
	 * Create a HilightRenderer. This sets the hilight color undefined to the color
	 * that is midway between the JTable's default selection color and default
	 * background color for the current look and feel.
	 */
	public HilightRenderer() {
		super();
		Color selectionBgColor = UIManager.getColor("Table.selectionBackground"); // NON-NLS
		if (selectionBgColor == null) {
			selectionBgColor = UIManager.getColor("Table.selectionColor"); // NON-NLS
		}
		if (selectionBgColor == null) {
			selectionBgColor = Color.BLUE;
		}
		Color bgColor = UIManager.getColor("Table.background"); // NON-NLS
		chooseHiliteColor(selectionBgColor, bgColor);
	}

	private void chooseHiliteColor(Color pSelectionBgColor, Color pBgColor) {
		Color clr = blendColors(pSelectionBgColor, pBgColor);
		setHiliteColor(clr);
	}
	...
	public void setHiliteColor(JTable tbl) {
		...
		float[] hsb = Color.RGBtoHSB(sb.getRed(), sb.getGreen(), sb.getBlue(), null);
		float sat = hsb[1];
		if (sat < 1.0F / 3.0f) {
			Color hiliteClr = blendColors(invert(b), sb);
			setHiliteColor(hiliteClr, invert(tbl.getForeground()));
		} else {
			Color hiliteClr = blendColors(b, sb);
			setHiliteColor(hiliteClr);
		}
	}

	private Color invert(Color clr) {
		return new Color(255 - clr.getRed(), 255 - clr.getGreen(), 255 - clr.getBlue());
	}

	private Color blendColors(Color b, Color sb) {
		assert b!=null : "Null color";
		assert sb!=null : "Null selection color";
...
//	                                String pTabBegin,
//	                                String pTabEnd) {
//		return renderHigh(pRndr, pCellRect, pNeedle, pTabBegin, pTabEnd, false);
//	}
	//
	public static String renderHigh(DefaultTableCellRenderer pRndr,
	                                Rectangle pCellRect,
		...
		int txtWid = SwingUtility.stringWidth(pRndr, pRndr.getFontMetrics(pRndr.getFont()), pHaystack);
		boolean clipped = availWidth < txtWid;
		if (clipped)
			haystack = SwingUtility.clipString(pRndr, pRndr.getFontMetrics(pRndr.getFont()), pHaystack, availWidth);
...
//		int lt=0;
		if (pAllowHTML) {
//			System.err.println("Massaging...");
//			ampLocs=new int[pHaystack.length()];
//			ltLocs=new int[pHaystack.length()];
//			int where = 0;
//			while ((where = pHaystack.indexOf('<', where)) >= 0)
//				ltLocs[lt++] = where++;
//			where = 0;
//			while ((where = pHaystack.indexOf('&', where)) >= 0)
//				ampLocs[amp++] = where++;
			String lowOrig = lowTxt;
			pHaystack = massage(pHaystack);
			lowTxt = massage(lowTxt);
			pNeedle = massage(pNeedle);
			haystack = massage(haystack);
			if (!lowTxt.equals(lowOrig))
				System.err.println("pHaystack to " + pHaystack);
		}
//		else
//			System.err.println("No Massage");
		...
		while (mark >= 0 && mark < haystackLen) {
			if (!pAllowHTML || isMarkValid(mark, mAmp, mLess, mGt)) {
...
//	}
	...
	public void listenToField(JTextField pFld, final JTable pTable) {
		DocumentListener lsnr = new DocumentListener() {
			public void insertUpdate(DocumentEvent e) { doHilite(e); }
	...
	public static String massage(String input) {
		...
		input = mGt.massage(input, mAmp, mGt);
//		String inputCopy = input;
//		// This one must be done first, because the 2nd one contains an ampersand.
//		input = input.replace("&", "&amp;");
//		inputCopy = sAmp.matcher(inputCopy).replaceAll(ampersandReplacement);
////		if (!input.equals(inputCopy))
////			System.err.println("Mismatch 1:\n  " + input + "\n  " + inputCopy);
//
//		input = input.replace("<", "&lt;");
////		inputCopy = sLt.matcher(inputCopy).replaceAll(lessThanReplacement);
////		if (!input.equals(inputCopy))
////			System.err.println("Mismatch 2:\n  " + input + "\n  " + inputCopy);
////		input = sAmp.matcher(input).replaceAll(ampersandReplacement);
////		input = sLt.matcher(input).replaceAll(lessThanReplacement);
		return input;
	}
...
//	static { System.err.println("LT:  " + lessThanReplacement + "\namp: " + ampersandReplacement); }
	...
	public static class EncodedSearcher {
		...
		public EncodedSearcher(String pNeedle, String pReplacement) {
			mText = pNeedle;
//		mNeedle = Pattern.compile(pNeedle, Pattern.LITERAL);
			mReplacement = pReplacement; //Matcher.quoteReplacement(pReplacement);
...
//		Matcher matcher = mNeedle.matcher(input);
//		String replacement = mReplacement.substring(where);
...
//	private static boolean isMarkValid(int ch, EncodedSearcher mn, EncodedSearcher... prev)
//	{
//		if (!mn.isMarkValid(ch))
//			return false;
//		return isMarkValid(ch, prev);
//	}
